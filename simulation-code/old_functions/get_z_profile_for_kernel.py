...
class ReturnCurrents:
    ...
    """
    def __init__(self, morphology_name, syn_pos, ext_sim_dict, cell_z_pos, zRotation, MEA, simulate_cell=True, calculate_mapping=True):
        ...
        phi = self.make_cell(ext_sim_dict, cell_z_pos, MEA)
        self.phi = phi 
    ...
    def make_cell(self, ext_sim_dict, cell_z_pos, MEA):
        ...
        cell_parameters = {
            ...
            'lambda_f': 6000,  # Higher number means higher spatial resolution of the cell
        ...
        cell.set_rotation(x=np.pi,z=self.zRotation*np.pi/4)
        ...
        if self.calculate_mapping:
            phi = self.make_mapping(cell, MEA, ext_sim_dict)
...
def get_phi_on_MEA_for_averaged_neuron(pos,index):
    ...
    cell_z_pos = 150  # z-position of cell, relative to MEA plane at 0 um
    top_to_soma = 230
    elec_x = []
    elec_y = []
    for x in range(-600,601,30):
        for y in range(-600,601,30):
            elec_x.append(x)
            elec_y.append(y)
    elec_params = {'slice_thickness': cell_z_pos+top_to_soma,
                   'elec_radius': 1.,
    ...
    synapse_pos = np.array([pos[0], pos[1], cell_z_pos+pos[2]])  # Synapse is inserted at closest cellular position to specified point (x,y,z)
    morphology_name = 'morphology_files/no_axonReal.hoc'    
    numRotation = 8
    phiMean = 0
    for zRotation in range(0,numRotation):
        rc = ReturnCurrents(morphology_name, synapse_pos, ext_sim_dict, cell_z_pos, zRotation, MEA,
                        calculate_mapping=True, simulate_cell=True)
        phiMean = phiMean + rc.phi;
        
    phiMean = phiMean/numRotation
    meanFolder = 'sim_zProfileofKernel'
