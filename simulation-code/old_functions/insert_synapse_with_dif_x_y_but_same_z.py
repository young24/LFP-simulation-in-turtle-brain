...
class ReturnCurrents:
    ...
    """
    def __init__(self, syn_pos, ext_sim_dict, cell_z_pos, rel_dist, index, MEA, simulate_cell=True, calculate_mapping=True):

        self.morphology_name = 'morphology_files/T_type.hoc'  # Could be .hoc or .swc and perhaps some other formats
    ...
    def make_cell(self, ext_sim_dict, cell_z_pos, MEA):
        ...
        if self.simulate_cell:
            ...
            imemName = 'imem_z'+str(self.rel_dist)+'_'+str(self.index)+'.npy'
            np.save(join(self.folder, imemName), cell.imem)
            np.save(join(self.folder, 'tvec.npy'), cell.tvec)
            vmemName = 'vmem_z'+str(self.rel_dist)+'_'+str(self.index)+'.npy' 
    ...
    def make_mapping(self, cell, MEA, ext_sim_dict, cell_z_pos):
        ...
        if hasattr(cell, 'imem'):
            ...
            phiName = 'phi_z'+str(self.rel_dist)+'_'+str(self.index)+'.npy'
            np.save(join(self.folder, phiName), MEA.phi)

if __name__ == '__main__':
    elec_params = {'slice_thickness': 500.,
                   'elec_radius': 1.,
    ...
    rel_dist = 151
    cell_parameters = {
        'morphology': 'morphology_files/T_type.hoc',
        'v_init': -65,
        'passive': False,
        'nsegs_method': 'lambda_f',
        'lambda_f': 400,  # Higher number means higher spatial resolution of the cell
    ...
    # Specify the position and rotation of the cell
    cell.set_pos(zpos=cell_z_pos)
    #cell.set_rotation(x=np.pi,z=0*np.pi)   # keep the rotation same to the one in the ReturnCurrent 
    ...
    syn_idx = cell.get_idx('dend',cell_z_pos+rel_dist-10,cell_z_pos+rel_dist+10)
          
    if (len(syn_idx)==0):
        ERRMSG = 'Cannot find suitable synapse for this z!'
        raise ValueError(ERRMSG)
    else:
        syn_position = np.vstack((cell.xmid[syn_idx],cell.ymid[syn_idx],cell.zmid[syn_idx]))
        syn_position = syn_position.T        
        if not os.path.isdir(folder):
            os.mkdir(folder)
        np.save(join(folder, 'syn_position_z_'+str(rel_dist)+'.npy'), syn_position)
        for index in range(0,len(syn_idx)):
            syn_pos = syn_position[index]
            rc = ReturnCurrents(syn_pos, ext_sim_dict, cell_z_pos, rel_dist, 
                                index, MEA, simulate_cell=True, calculate_mapping=True)
