...
def make_cell(morphologyPath,soma_z=150,upsideDown=True):
    ...
    cell_parameters = {
            ...
            'max_nsegs_length': 10,  # mm, max segment length for method 'fixed_length'
...
def average_phi_radically(phi,MEA,center,folderPath,rResolution=30): # syn position is the centerPos
...
def sum_over_same_z_zone(synZ,folderPath,soma_to_bottom_MEA=150,zResolution = 10,rResolution=30):
    ...
    phiList = [np.load(folderPath+'/phiProfile'+str(i)+'.npy') for i in range(0,sum(binCount))]
    ...
    for idx in range(0,np.shape(phiKernel[0])[0]):
        ...
        np.save(join(folderPath,'phiKernel_soma_above_MEA_z'+str(soma_to_bottom_MEA-idx*zResolution)+'.npy'),kernelPotential)
    ...
    np.save(join(folderPath,'coordinates.npy'),(zCoordinate,rCoordinate))
...
if __name__ == '__main__':
    ...
    morphologyPath = 'morphology_files/T22N5_noAxon.hoc'
    folderPath = '//storage/laur/Data_3/Wenbin_Yang/Sim_DIY/inhi_kernel'
    if not os.path.isdir(folderPath): 
        os.mkdir(folderPath)
    # build the cell
    synPos = get_inserted_synPos(morphologyPath,soma_z=0,upsideDown=True) # to avoid some trouble in following function   
    # insert the synapse, parallel computing 
    pool = multiprocessing.Pool(processes=32)
    
    index=range(0,np.shape(synPos)[0]) # syn number    
    pool.map(func_star, itertools.izip(index, itertools.repeat(morphologyPath),itertools.repeat(folderPath)))
    
    pool.close()
    pool.join()
    
    sum_over_same_z_zone(synPos[:,2],folderPath,soma_to_bottom_MEA=150)
