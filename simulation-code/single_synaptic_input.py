...
class ReturnCurrents:
    ...
    def make_cell(self, soma_z):
		...
		# save as .mat files
            sio.savemat(join(self.folder, 'imem.npy'), cell.imem)
            sio.savemat(join(self.folder, 'tvec.npy'), cell.tvec)
            sio.savemat(join(self.folder, 'vmem.npy'), cell.vmem)
            sio.savemat(join(self.folder, 'syn_i.npy'), synapse.i)
    ...
    def make_mapping(self, cell, MEA, ext_sim_dict):
        ...
        if hasattr(cell, 'imem'):
            ...
            sio.savemat(join(self.folder, 'phi.npy'), MEA.phi)
...
if __name__ == '__main__':
    ...
    syn_pos = np.array([0, 0, soma_z+250])  # Synapse is inserted at closest cellular position to specified point (x,y,z)
    ...
    isPassive = False
    upsideDown = True # if morphology is upsideDown
