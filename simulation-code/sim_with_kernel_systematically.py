...
def calc_LFP(t,zShift,MEA_to_soma,folder):
...
#    MEA_to_soma = 2 * zResolution # x10 um
    ...
    folderName = 'pyramidal'
    kernelPotential = np.load(folderName+'/KernelPotential_soma_MEA_z'+str(MEA_to_soma)+'.npy')
    coordinates = np.load(folderName+'/coordinates.npy')
    axonSyn = np.load('inputSynPos/V6N2_resolution10.npy')
...
if __name__ == '__main__':
    ...
    for MEA_to_soma in range(80,121,10):
        for zShift in range(150,501,50):
            pool = multiprocessing.Pool(processes=8)
            
            
            folderPath = 'V6N2_L2_'+str(MEA_to_soma)+'_L1_'+str(MEA_to_soma+zShift)
            if not os.path.isdir(folderPath):
                os.mkdir(folderPath)
            t = range(0,401)
            pool.map(func_star, itertools.izip(t, itertools.repeat(zShift),itertools.repeat(MEA_to_soma),itertools.repeat(folderPath)))
